...
async function fetchAcademicPapers(query: string, mode: string = "smart") {
    ...
    if (res.ok) {
      ...
      for (const item of data.message?.items || []) {
        ...
        results.push({
          ...
          authors,
          year: exactDate,
          source: item["container-title"]?.[0] ? `頂刊: ${item["container-title"][0].slice(0, 20)}` : "CrossRef (國際頂刊)",
          link,
          citations,
      ...
      if (idList.length > 0) {
        ...
        if (sumRes.ok) {
          ...
          for (const pmid of idList) {
            ...
            const journal = doc.source || "PubMed 生醫頂刊";
            const fp = title.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 50);
            results.push({
              id: `pmid_${pmid}`,
              fingerprint: fp,
  ...
  const scored = uniqueList.map(p => {
    ...
    let modeScore = 0;
    if (mode === "top_tier") {
      modeScore = Math.min(Math.floor(p.citations / 3), 50) + (p.is_top_journal ? 30 : 0);
    } else if (mode === "free_only") {
      modeScore = Math.min(Math.floor(p.citations / 10), 15) + (p.is_open_access ? 30 : -20);
    } else {
      modeScore = Math.min(Math.floor(p.citations / 5), 30) + (p.is_top_journal ? 15 : 0) + (p.is_open_access ? 10 : 0);
    }
